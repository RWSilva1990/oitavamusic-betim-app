...
replace(
    'src/lib/push-notifications.functions.ts',
...
replace(
    'src/routes/api/mobile/push.ts',
    "  token: z.string().min(8).max(4096).optional(),\n}).superRefine",
    "  token: z.string().min(8).max(4096).optional(),\n  platform: z.enum(['web', 'android']).optional(),\n}).superRefine",
)

p = Path('src/lib/push-notifications.server.ts')
text = p.read_text()
text, n = re.subn(
    r"type InstallationTarget = \{\n  value: string;\n  type: 'fid' \| 'token';\n  path: string;\n  memberId: string;\n\};",
    "type InstallationTarget = {\n  value: string;\n  type: 'fid' | 'token';\n  path: string;\n  memberId: string;\n  platform: 'web' | 'android';\n};",
    text,
    count=1,
)
if n != 1:
    raise SystemExit('push-notifications.server.ts: InstallationTarget block mismatch')

text, n = re.subn(
    r"function installationFromDocument\(document: any, memberIdOverride\?: string\): InstallationTarget \| null \{.*?\n\}\n\nasync function findInstallationsByMemberIds",
...
if n != 1:
    raise SystemExit('push-notifications.server.ts: installationFromDocument block mismatch')

text, n = re.subn(
    r"data: \{ fid\?: string; token\?: string \},",
    "data: { fid?: string; token?: string; platform?: 'web' | 'android' },",
    text,
    count=1,
)
if n != 1:
    raise SystemExit('push-notifications.server.ts: register signature mismatch')

old = """        token: target.type === 'token' ? target.value : '',
        memberId,"""
new = """        token: target.type === 'token' ? target.value : '',
        platform: data.platform || (target.type === 'fid' ? 'web' : 'android'),
        memberId,"""
if text.count(old) != 1:
    raise SystemExit('push-notifications.server.ts: storage block mismatch')
text = text.replace(old, new, 1)

old = """  console.info('[push-register]', JSON.stringify({ memberId, targetType: target.type }));
  return { success: true, memberId, targetType: target.type };"""
new = """  const platform = data.platform || (target.type === 'fid' ? 'web' : 'android');
...
if text.count(old) != 1:
    raise SystemExit('push-notifications.server.ts: register result block mismatch')
text = text.replace(old, new, 1)

start = text.index("  const nativeTargets = installations.filter((item) => item.type === 'token');")
end = text.index("\n  for (let i = 0; i < nativeTargets.length; i += 500) {", start)
web_block = """  const nativeTargets = installations.filter((item) => item.platform === 'android');
...
p = Path('src/lib/notification-center.server.ts')
text = p.read_text()
start = text.index('type PushTarget =')
end = text.index('\nfunction formatScaleDate', start)
replacement = """type PushTarget = { token: string; memberId: string; path: string; platform: 'android' | 'web' };
...
if text.count('sendNative(caller.app, allowed, {') != 1:
    raise SystemExit('notification-center.server.ts: notice send call mismatch')
text = text.replace('sendNative(caller.app, allowed, {', 'sendPush(caller.app, allowed, {', 1)
if text.count('sendNative(app, pending, {') != 1:
    raise SystemExit('notification-center.server.ts: reminder send call mismatch')
text = text.replace('sendNative(app, pending, {', 'sendPush(app, pending, {', 1)
p.write_text(text)
