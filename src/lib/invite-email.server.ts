...
async function assertAdmin(idToken: string) {
  ...
  if (adminEmails().includes(callerEmail)) return { auth, callerEmail };

  const access = await firestoreRest(
    app,
    `/accessUsers/${encodeURIComponent(callerEmail)}`,
    {},
    { allowNotFound: true },
  );
                      ...
                      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;margin:0 0 28px;background:#F8F7FF;border:1px solid #E8E2FF;border-radius:16px;">
                        <tr>
                          ...
                          <td width="25%" align="center" valign="top" style="padding:18px 6px;">
                            <div style="font-size:23px;line-height:1;margin-bottom:9px;">🔔</div>
                            <div class="feature-label" style="font-size:11px;line-height:1.45;font-weight:700;color:#111827;">Receba avisos<br />de escala</div>
                          </td>
                        </tr>
                      </table>
...
export async function sendInvitationEmailForToken(email: string, idToken: string) {
  const clean = email.trim().toLowerCase();
  const { auth } = await assertAdmin(idToken);
  const appUrl = (env('APP_URL') || 'https://oitavamusicbetim.vercel.app').replace(/\/$/, '');
  const inviteLink = await auth.generateSignInWithEmailLink(clean, {
    url: `${appUrl}/convite`,
