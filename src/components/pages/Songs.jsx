...
import { genId, getYtId, normalizeStr } from '@/lib/db';
...
function findExistingSongIndex(list, imported) {
  const importedId = String(imported?.sourceId || '').trim();
  if (importedId) {
    const byId = list.findIndex((song) => String(song?.id || '').trim() === importedId);
    if (byId >= 0) return byId;
  }

  const importedName = normalizeStr(imported?.name);
  if (importedName) {
    const byName = list.findIndex((song) => normalizeStr(song?.name) === importedName);
    if (byName >= 0) return byName;
  }

  const importedYoutubeId = getYtId(String(imported?.youtubeUrl || '').trim());
  if (importedYoutubeId) {
    const byYoutube = list.findIndex((song) => getYtId(String(song?.youtubeUrl || '').trim()) === importedYoutubeId);
    if (byYoutube >= 0) return byYoutube;
  }
...
export default function SongsPage() {
  ...
  const save = () => {
    ...
    const isDup = songs.some((s) => normalizeStr(s.name) === normalizeStr(form.name) && (modal === 'add' || s.id !== form.id));
    if (isDup) {
    ...
    Papa.parse(file, {
      ...
      complete: ({ data }) => {
        const parsedRows = data.map((row) => {
          ...
          const idk = keys.find((k) => /^\s*id\s*$/i.test(k.replace(/^\uFEFF/, '')));
          const nk = keys.find((k) => /nome|name|musica|titulo|title/i.test(k)) || keys[0];
          const uk = keys.find((k) => /url|link|youtube/i.test(k));
          ...
          const importedSignature = tk ? String(row[tk] || '').trim() : '';
          return {
            sourceId: idk ? String(row[idk] || '').trim() : '',
            name: String(row[nk] || '').trim(),
            youtubeUrl: uk ? String(row[uk] || '').trim() : '',
            bpm: bk ? String(row[bk] || '').trim() : '',
            originalKey: ok ? String(row[ok] || '').trim() : '',
        ...
        for (const row of parsedRows) {
          const duplicateInFile = findExistingSongIndex(
            rows.map((item) => ({ ...item, id: item.sourceId || '' })),
            row,
          );
          if (duplicateInFile >= 0) rows[duplicateInFile] = { ...rows[duplicateInFile], ...row };
          else rows.push(row);
        }
        setPreview(rows);
      },
  ...
  const doImport = () => {
    ...
    setSongs((current) => {
      ...
      for (const imported of preview) {
        const index = findExistingSongIndex(updated, imported);
        ...
        if (index >= 0) {
          ...
          const { sourceId, ...newSong } = imported;
          updated.push({ ...newSong, audios: [], id: genId() });
        }
      }
    ...
    <div style={{ padding: 24, maxWidth: 860 }}>
      ...
      {!readOnly && importModal && <Modal title="Importar/Atualizar Músicas via CSV" onClose={() => { setImportModal(false); setPreview([]); }} wide>
        <div style={{ padding: 14, background: C.bgInput, borderRadius: 8, marginBottom: 16, fontSize: 13, color: C.textSecondary, lineHeight: 1.7 }}>
          <strong style={{ color: C.accent }}>Atualização segura:</strong> CSVs exportados pelo Oitava incluem uma coluna <code>id</code> para reconhecer cada música com segurança. Não altere essa coluna. Arquivos antigos sem <code>id</code> continuam compatíveis e serão identificados pelo nome e, quando possível, pelo vídeo do YouTube. Campos vazios não apagam informações existentes.
        </div>
