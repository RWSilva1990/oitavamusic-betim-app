...
export default function MemberInvitePanel() {
  ...
  useEffect(() => {
    ...
    refresh();
    const refreshWhenVisible = () => { if (document.visibilityState === 'visible') refresh({ silent: true }); };
    const refreshWhenFocused = () => refresh({ silent: true });
  ...
  const copyWhatsAppNotice = async () => {
    ...
    try {
      if (navigator.clipboard?.writeText) await navigator.clipboard.writeText(notice);
      else { const textarea = document.createElement('textarea'); textarea.value = notice; textarea.style.position = 'fixed'; textarea.style.opacity = '0'; document.body.appendChild(textarea); textarea.focus(); textarea.select(); document.execCommand('copy'); document.body.removeChild(textarea); }
      setNoticeCopied(true);
    } catch { setNoticeCopied(false); setMessage('Não foi possível copiar o aviso automaticamente.'); }
  };

  const syncAccessDirectory = async () => {
    setSyncResult(null); setSyncingAccess(true);
    try { setSyncResult(await auth.syncMemberDirectory(members)); }
    catch (e) { setSyncResult({ error: e?.message || 'Não foi possível sincronizar os acessos.' }); }
    finally { setSyncingAccess(false); }
  };

  const accept = async (registration) => {
    setBusyUid(registration.uid); setError('');
    try {
      const result = await acceptPendingRegistration(registration.uid);
      if (Array.isArray(result?.members)) setMembers(result.members);
      setRegistrations((rows) => rows.filter((r) => r.uid !== registration.uid));
      await refresh({ silent: true });
    } catch (e) { setError(e?.message || 'Não foi possível concluir o cadastro.'); }
    finally { setBusyUid(''); }
  };
  ...
  return <>
    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
      ...
      <Btn variant="secondary" disabled={syncingAccess} onClick={syncAccessDirectory}><Link2 size={15} />{syncingAccess ? 'Sincronizando...' : 'Sincronizar acessos'}</Btn>
    </div>
    {syncResult && <div style={{ marginTop: 10, padding: '10px 12px', background: C.bgInput, borderRadius: 8, fontSize: 12, color: syncResult.error ? C.danger : C.textSecondary, lineHeight: 1.6 }}>{syncResult.error ? syncResult.error : <><strong style={{ color: C.textPrimary }}>Sincronização concluída.</strong>{' '}{syncResult.linked} novo(s) vínculo(s), {syncResult.alreadyLinked} já vinculado(s), {syncResult.withoutEmail} sem e-mail, {syncResult.invalidEmail} e-mail(s) inválido(s), {syncResult.duplicates} e-mail(s) duplicado(s) e {syncResult.conflicts} conflito(s).{(syncResult.duplicates > 0 || syncResult.conflicts > 0) && <div style={{ marginTop: 5, color: C.accent }}>Duplicidades e conflitos foram ignorados automaticamente para não trocar o acesso de ninguém.</div>}</>}</div>}
    <div className="card" style={{ marginTop: 12, marginBottom: 18 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}><UserCheck size={17} color={C.accent} /><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontWeight: 800, color: C.textPrimary }}>Cadastros recebidos</div><div style={{ fontSize: 12, color: C.textSecondary }}>Dados preenchidos pelas pessoas que receberam seu convite.</div></div><Btn variant="ghost" title="Atualizar" disabled={refreshing} onClick={() => refresh()}><RefreshCw size={14} style={refreshing ? { animation: 'spin 0.8s linear infinite' } : undefined} /></Btn></div>
