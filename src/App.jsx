...
const LIGHT = {
  ...
  accent:       '#B8861A',
  accentDark:   '#7A5A0E',
  accentGlow:   'rgba(184,134,26,0.10)',
  border:       '#D8DFE8',
  textPrimary:  '#1A2233',
  ...
  success:      '#2E8A52',
  blue:         '#3060C0',
  modalOverlay: 'rgba(30,40,70,0.45)',
...
const makeCSS = (C) => `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  html, body { background: ${C.bg}; font-family: 'Nunito', sans-serif; color: ${C.textPrimary}; transition: background 0.2s, color 0.2s; }
  ...
  .sidebar {
    ...
    transition: transform 0.28s cubic-bezier(.4,0,.2,1), background 0.2s, border-color 0.2s;
  ...
  .modal-overlay {
    ...
    background: ${C.modalOverlay}; backdrop-filter: blur(6px);
  ...
  .section-header {
    ...
    color: ${C.textSecondary}; text-transform: uppercase; letter-spacing: 1.2px;
...
`;
// Mutable ref so small shared components (Btn, Modal, Avatar…) always get current theme
// without needing prop-drilling into every one of them.
let _C = DARK;
const getC = () => _C;
...
function buildWhatsAppText(sc, members, groups, songs) {
  ...
  if (scMembers.length > 0) {
    ...
    scMembers.forEach(x => {
      ...
      txt += `• ${x.member.name}${x.isSub ? '' : ''}${roleLabel ? ` — ${roleLabel}` : ''}\n`;
    });
    txt += `\n`;
  }
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box" style={{ maxWidth: wide ? 680 : 460 }} onClick={e => e.stopPropagation()}>
        <div className="modal-header">
          <h2 style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 900, fontSize: 16, color: C.accent }}>{title}</h2>
...
function HomePage({ C, counts, scales, members, groups, onNav }) {
...
function MembersPage({ C, members, setMembers }) {
...
function GroupsPage({ C, groups, setGroups, members }) {
  const [modal, setModal] = useState(null);
  const [confirm, setConfirm] = useState(null);
...
function SongsPage({ C, songs, setSongs }) {
  const [search, setSearch] = useState('');
  const [modal, setModal] = useState(null);
  const [confirm, setConfirm] = useState(null);
...
function ArchivedSection({ C, archived, ScaleCard }) {
...
function ScalesPage({ C, scales, setScales, members, groups, songs }) {
  const [modal, setModal] = useState(null);
      ...
      {scales.length === 0 ? (
        ...
        return (
          <div style={{ display: 'grid', gap: 16 }}>
            ...
            {/* Archived */}
            {archived.length > 0 && (
              <ArchivedSection C={C} archived={archived} ScaleCard={ScaleCard} />
            )}
          </div>
        );
      })()}
...
function ReportsPage({ C, scales, songs }) {
  const today = new Date().toISOString().split('T')[0];
...
export default function App() {
  ...
  _C = C; // keep global ref in sync for shared components
  const GLOBAL_CSS = useMemo(() => makeCSS(C), [dark]);

  const toggleTheme = () => setDark(d => {
    try { localStorage.setItem('omTheme', d ? 'light' : 'dark'); } catch {}
    return !d;
  });
  ...
  // Auto-refresh every 30s when window has focus
  useEffect(() => {
    ...
    <>
      ...
      <div className={`sidebar${sideOpen ? ' open' : ''}`}>
        ...
        <div style={{ padding: '12px 10px', borderTop: `1px solid ${C.border}` }}>
          <button
            onClick={toggleTheme}
            style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', borderRadius: 8, background: C.bgHover, border: `1px solid ${C.border}`, color: C.textSecondary, cursor: 'pointer', fontSize: 13, fontFamily: 'Nunito, sans-serif', fontWeight: 600, transition: 'all 0.15s' }}
          >
            {dark ? <Sun size={15} color={C.accent} /> : <Moon size={15} color={C.accent} />}
            {dark ? 'Tema Claro' : 'Tema Escuro'}
            <span style={{ marginLeft: 'auto', fontSize: 10, padding: '2px 7px', borderRadius: 20, background: C.accentGlow, color: C.accent, border: `1px solid ${C.accent}44` }}>
              {dark ? '☀️' : '🌙'}
            </span>
          </button>
          <div style={{ marginTop: 10, fontSize: 11, color: C.textSecondary, lineHeight: 1.5, paddingLeft: 2 }}>
      ...
      {/* Main */}
      <div className="main-content" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: C.bg }}>
        {/* Topbar */}
        <div style={{ height: 54, background: C.bgSecondary, borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', padding: '0 18px', gap: 12, position: 'sticky', top: 0, zIndex: 100 }}>
          ...
          {/* Theme toggle in topbar (mobile) */}
          <button onClick={toggleTheme} style={{ background: 'none', border: 'none', cursor: 'pointer', color: C.textSecondary, display: 'flex', alignItems: 'center', padding: '4px 6px', borderRadius: 6 }}>
        ...
        {/* Page */}
        <div style={{ flex: 1 }}>
          {page === 'home'    && <HomePage C={C} counts={{ members: members.length, groups: groups.length, songs: songs.length, scales: scales.length }} scales={scales} members={members} groups={groups} onNav={nav} />}
          {page === 'members' && <MembersPage C={C} members={members} setMembers={setMembers} />}
          {page === 'groups'  && <GroupsPage C={C} groups={groups} setGroups={setGroups} members={members} />}
          {page === 'songs'   && <SongsPage C={C} songs={songs} setSongs={setSongs} />}
          {page === 'scales'  && <ScalesPage C={C} scales={scales} setScales={setScales} members={members} groups={groups} songs={songs} />}
          {page === 'reports' && <ReportsPage C={C} scales={scales} songs={songs} />}
        </div>
      </div>
    </>
  );
